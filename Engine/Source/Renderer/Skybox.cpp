...
Skybox::Skybox(std::string path) : path(path)
{
    ...
    if (!data)
    {
        std::cout << "Couldn't locate file " << path << std::endl;
        return;
    }
    ...
    hdrToCubemapShader = std::make_unique<Shader>("Assets/Shaders/Skybox/hdr_to_cubemap.glsl", "HDR_TO_CUBEMAP");
    ...
    irradianceMapShader = std::make_unique<Shader>("Assets/Shaders/Skybox/irradiance_skybox.glsl", "IRRADIANCE");
    ...
    prefilterShader = std::make_unique<Shader>("Assets/Shaders/Skybox/prefilter.glsl", "PREFILTER");
    ...
    brdfShader = std::make_unique<Shader>("Assets/Shaders/Skybox/brdf.glsl", "BRDF");
    brdfShader->Bind();
    glViewport(0, 0, 512, 512);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    DrawQuad();
    ...
    skyboxShader = std::make_unique<Shader>("Assets/Shaders/Skybox/skybox.glsl", "SKYBOX");

    reflectionLod = 2.0f;
    enableReflection = true;

    std::cout << "Skybox " << name << "loaded successfully" << std::endl;
...
void Skybox::SetUpCube()
{
    ...
    cubeVbo = std::make_shared<VertexBuffer>(vertices, 108);

    VertexBufferLayout vertexBufferLayout = {};
    vertexBufferLayout.attributes.push_back(VertexBufferAttribute{ 0, 3, 0 });
    vertexBufferLayout.stride += 3 * sizeof(float);

    cubeVbo->SetLayout(vertexBufferLayout);
...
void Skybox::SetUpQuad()
{
    ...
    quadVbo = std::make_shared<VertexBuffer>(vertices, 20);

    VertexBufferLayout quadLayout = {};
    quadLayout.attributes.push_back(VertexBufferAttribute{ 0, 3, 0 });
    quadLayout.stride += 3 * sizeof(float);
    quadLayout.attributes.push_back(VertexBufferAttribute{ 1, 2, quadLayout.stride });
    quadLayout.stride += 2 * sizeof(float);
